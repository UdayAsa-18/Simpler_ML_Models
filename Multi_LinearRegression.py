...
class MultiLinearRegression:
    ...
    def fit(self, X, y, batch_size=32, regularization=0, max_epochs=100, patience=3):
        ...
        if(y.ndim>1):
            self.bias = np.random.randn(y.shape[1])
        else:
            self.bias = (np.random.rand() * 2) - 1

        #print(self.bias.shape) #new
        ...
        for epoch in range(max_epochs):
               ...
               for i in range(0, data_train, batch_size):
                   ...
                   # print("batch_X",batch_train_X.shape) #new
                   # print("batch_y",batch_train_y.shape) #new

                    y_hat = np.dot(batch_train_X,self.weights) + self.bias # y = mx+c form 

                    # print("y_hat is",y_hat.shape) #new
                    ...
                    # print("gradient Weights",gradient_weights.shape)#new

                    gradient_bias = np.sum(y_hat - batch_train_y)/batch_size

                    # print("gradient bias",gradient_bias.shape)#new
